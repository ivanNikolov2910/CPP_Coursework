...
double calculateExpensesForFlight(const Plane &plane, const Flight &flight) {
    ...
    double staffSalary = flight.getDistance() * (
            (static_cast<int>(flight.getDistance() / 1000) + 1 * STEWARD_SALARY_PER_FLIGHT) * plane.getStewardCount() +
            (static_cast<int>(flight.getDistance() / 1000) + 1 * PILOT_SALARY_PER_FLIGHT)) * plane.getPilotCount();
