...
ResultCode ListPlanes(std::vector<Plane> *planes)
{
    ...
    Plane *newPlane;
    while (newPlane->get(file))
    {
        if (instanceof <CargoPlane>(newPlane))
        {
            CargoPlane *cp = (CargoPlane *)newPlane;
            planes->push_back(*cp);
        }
        else if (instanceof <PassengerPlane>(newPlane))
        {
            PassengerPlane *pp = (PassengerPlane *)newPlane;
            planes->push_back(*pp);
        }
    }
...
Plane *readPlaneData(const std::vector<Plane> &planes)
{
    ...
    if (cargoWeight > 0)
    {
        newPlane = new CargoPlane(id, manufacturer, brand, runwayLength, fuelPerKilometer, pilotCount, stewardCount, cargoWeight);
    }
    else
    {
        newPlane = new PassengerPlane(id, manufacturer, brand, runwayLength, fuelPerKilometer, pilotCount, stewardCount, businessSeat,
                                      passengerSeat);
    }
